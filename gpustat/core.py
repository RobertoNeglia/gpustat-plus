...
class GPUStatCollection(Sequence[GPUStat]):
    ...
    def collect_system_info(self):
        ...
        try:
            ...
            # Also collect per-core CPU count for reference
        ...
        try:
            ...
            self.system_memory_used = memory_info.used // MB
            self.system_memory_total = memory_info.total // MB
            self.system_memory_percent = memory_info.percent
        except Exception:
            self.system_memory_used = None
            self.system_memory_total = None
            self.system_memory_percent = None
    ...
    def _format_system_info(self, t_color, gpuname_width):
        ...
        # Calculate same width as GPU display for alignment
        name_width = gpuname_width or DEFAULT_GPUNAME_WIDTH

        # Color settings similar to GPU display
        ...
        if self.system_memory_used is not None and self.system_memory_total is not None:
            ...
            memory_percent = (self.system_memory_used / self.system_memory_total) * 100

            mem_used_color = _conditional(
                lambda: memory_percent < 80, t_color.bold_yellow, t_color.bold_red
            )
    ...
    def new_query(debug=False, id=None) -> "GPUStatCollection":
        ...
        collection = GPUStatCollection(gpu_list, driver_version=driver_version)
    ...
    def print_formatted(
        ...
        *,
        ...
        # system information
        if show_system:
            system_info = self._format_system_info(t_color, gpuname_width)
            if system_info:
                fp.write(system_info)
                fp.write(eol_char)
    ...
    def jsonify(self):
        ...
        if hasattr(self, "system_cpu_percent") and (
            ...
            memory_percent = None
            if (
                self.system_memory_used is not None
                and self.system_memory_total is not None
                and self.system_memory_total > 0
            ):
                memory_percent = (
